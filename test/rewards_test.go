...
func callbackStr(orderNo string, metaData string, amount int) string {
	...
	out := fmt.Sprintf(jsonStream, metaData, orderNo, amount);
	return out
}

func testCallbackStr(orderNo string, orderId string, amount int) string {
	return callbackStr(orderNo, "\"orderId\": " + orderId, amount);
}
